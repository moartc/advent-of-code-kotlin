...
fun part1(input: List<String>, pointToCheck: Int): Int {
    ...
    val res = map.flatMap { pair ->
        (min - maxDist..max + maxDist).mapNotNull { x ->
            val p = x to pointToCheck
            if (distance(pair.first, p) <= distance(pair.first, pair.second)) p else null
        }
    }.toSet().size
...
fun part2(input: List<String>, maxVal: Int): Long {
    ...
    val pair = map.firstNotNullOf {
        pointsOutside(it.first, it.second, maxVal).firstOrNull { pair -> !pointToDist.any { p -> distance(pair, p.first) <= p.second } }
    }
...
fun pointsOutside(p1: Pair<Int, Int>, p2: Pair<Int, Int>, maxVal: Int): Sequence<Pair<Int, Int>> {
    ...
    val seqEdge1 = generateSequence(p1.first to p1.second - dist - 1) { p -> p.first + 1 to p.second + 1 }.takeWhile { it.second <= p1.second }
        .filter { it.first in 0..maxVal && it.second >= 0 && it.second <= maxVal }
...
fun distance(p0: Pair<Int, Int>, p1: Pair<Int, Int>): Int = (kotlin.math.abs(p0.first - p1.first) + kotlin.math.abs(p1.second - p0.second))